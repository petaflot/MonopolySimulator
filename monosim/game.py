...
class Game:
	def __init__(self, admin, board, bank, community_cards_deck, chance_cards_deck, name = 'Monopoly'):
	...
	async def game_loop(self, players):
		...
		await self.broadcast(f"{self.name}: game starts : unless you are the game admin, enter any command to leave the lobby and enter the game.")
					...
					while True:
						...
						try:
							if any([c.startswith(cmd[0]) for c in (b'pass', b'roll', b'buy', b'pay')]):
								...
								elif b'pass'.startswith(cmd[0]) or (b'end'.startswith(cmd[0]) and b'turn'.startswith(cmd[1])):
									if player._jail_count:
